...
func TestCollectionScheduleRequestSubscriberJob(t *testing.T) {
	...
	pub := cdg.NewCollectionInitPublisher(ctx, conn, logger, factory)
	...
	fakePayload.AcknowledgeStub = func(err ...error) error {
		...
		assert.Equal(1, fakeRepo.InsertCallCount())
	...
	go func() {
		cdg.CollectionScheduleRequestSubscriberJob(ctx, eventChannel, (*cdg.CollectionInitPublisher)(pub), logging.GetLogger(), repo)
	}()
...
func TestInsertFailure(t *testing.T) {
	...
	fakePayload.AcknowledgeStub = func(err ...error) error {
		...
		assert.Equal(1, fakeRepo.InsertCallCount())
		assert.Equal(0, fakePub.PublishCallCount())
		assert.Equal(1, fakePayload.AcknowledgeCallCount())
		cancel()
		waitGroup.Done()
		return nil
